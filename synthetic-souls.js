...
function initAudioPlayer() {
  ...
  function loadTrack(trackIndex) {
    ...
    const visualizerSrc = currentItem.getAttribute('data-visualizer');
    
    // Set audio source and load
    mainAudio.src = trackSrc;
    mainAudio.load();
    
    // Update artwork: visualizer drop-in or static image
    const trackId = currentItem.getAttribute('data-track') || '';
    playerArtwork.setAttribute('data-track', trackId);
    if (visualizerSrc && visualizerFrame) {
      playerArtwork.classList.add('has-visualizer');
      visualizerFrame.src = visualizerSrc;
    } else {
      playerArtwork.classList.remove('has-visualizer');
      if (visualizerFrame) visualizerFrame.src = 'about:blank';
